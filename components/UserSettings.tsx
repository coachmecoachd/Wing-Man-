...
<<<<<<< HEAD
                <h2 className="text-5xl font-extrabold text-white tracking-tight">Settings</h2>
                <p className="mt-3 text-xl text-slate-400">Personalize your experience.</p>
            </div>

            <div className="bg-secondary p-10 rounded-[2rem] shadow-2xl border border-tertiary">
                <form onSubmit={handleSubmit} className="space-y-10">
                    <div className="flex flex-col items-center">
                        <div className="relative group cursor-pointer" onClick={handleUploadClick}>
                            <div className="w-40 h-40 rounded-full border-4 border-tertiary group-hover:border-accent transition-all duration-300 overflow-hidden bg-primary shadow-2xl relative">
                                {formData.avatarUrl ? (
                                    <img src={formData.avatarUrl} alt="Profile" className="w-full h-full object-cover" />
                                ) : (
                                    <div className="w-full h-full flex items-center justify-center text-5xl font-bold text-white bg-stone-800">
=======
...
<<<<<<< HEAD
                            <button type="button" onClick={handleUploadClick} className="flex items-center gap-2 text-sm bg-tertiary px-5 py-2.5 rounded-xl text-slate-200 hover:bg-stone-700 hover:text-white transition-all shadow-md font-bold border border-stone-700">
                                <Upload size={16} /> Upload
                            </button>
                            <button type="button" onClick={handleRandomizeAvatar} className="flex items-center gap-2 text-sm bg-tertiary px-5 py-2.5 rounded-xl text-slate-200 hover:bg-stone-700 hover:text-white transition-all shadow-md font-bold border border-stone-700">
=======
...
<<<<<<< HEAD
                        {error && <p className="text-red-400 text-sm mt-4 animate-fade-in bg-red-900/20 px-4 py-2 rounded-lg border border-red-500/20 font-medium">{error}</p>}
                        {successMsg && <p className="text-green-400 text-sm mt-4 animate-fade-in flex items-center gap-2 bg-green-900/20 px-4 py-2 rounded-lg border border-green-500/20 font-medium"><CheckCircle2 size={16} /> {successMsg}</p>}
=======
...
<<<<<<< HEAD
                            <p className="mt-2 text-xs text-slate-500 font-medium">Used for quick recommendations in the Date Planner.</p>
                        </div>
                    </div>

                    <div className="pt-8 border-t border-tertiary flex flex-col sm:flex-row gap-6 justify-between items-center">
                         <button type="button" onClick={onReplayTutorial} className="text-sm text-slate-400 hover:text-white hover:underline transition-colors">
=======
